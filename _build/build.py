...
<p id="sharemsg" class="sharemsg" role="status"></p>
    ...
    function submit(){
      ...
      fetch('https://a.klaviyo.com/client/subscriptions/?company_id=XvWZZX',{method:'POST',
        ...
        if(r.status===202){done=true;inp.style.display='none';btn.style.display='none';
    ...
    function hot(){if(done)return;
      ...
      pulse();box.scrollIntoView({behavior:'smooth',block:'center'});}
    ['save','follow'].forEach(function(id){var el=document.getElementById(id);
      if(el)el.addEventListener('click',function(){if(!armed){armed=true;setTimeout(hot,900);}});});
    document.addEventListener('visibilitychange',function(){
      if(armed&&!done&&document.visibilityState==='visible')pulse();});
    })();
    </script>
